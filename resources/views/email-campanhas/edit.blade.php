...
@section('title', 'Editar Campanha — WR Assessoria')

@section('content')
<div class="w-full mx-auto py-6 px-4">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('email-campanhas.show', $campanha->id) }}" class="text-gray-400 hover:text-brand">
            <i class="fa-solid fa-arrow-left"></i>
        </a>
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-slate-100"><i class="fa-solid fa-pen-to-square"></i> Editar Campanha</h1>
            <p class="text-gray-700 dark:text-gray-300">{{ $campanha->titulo }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('email-campanhas.update', $campanha->id) }}" id="form-campanha">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white dark:bg-slate-800 rounded shadow p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="titulo" value="{{ old('titulo', $campanha->titulo) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="assunto" value="{{ old('assunto', $campanha->assunto) }}"
                ...
                {{-- Editor de conteúdo --}}
                <div class="bg-white dark:bg-slate-800 rounded shadow p-5">
                    <h2 class="text-sm font-semibold text-gray-700 dark:text-slate-200 uppercase tracking-wide mb-3">Conteúdo do E-mail <span class="text-red-500">*</span></h2>
                    <textarea name="conteudo_html" id="conteudo-html" rows="18"
                              placeholder="Cole ou escreva o HTML do e-mail aqui..."
                              class="w-full border border-gray-300 dark:border-slate-600 rounded px-3 py-2 text-xs font-mono text-gray-800 dark:text-slate-200 bg-white dark:bg-slate-700 focus:outline-none focus:ring-1 focus:ring-brand resize-y">{{ old('conteudo_html', $campanha->conteudo_html) }}</textarea>
                </div>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white dark:bg-slate-800 rounded shadow p-5">
                    ...
                    <input type="datetime-local" name="enviar_em"
                           value="{{ old('enviar_em', $campanha->enviar_em?->format('Y-m-d\TH:i')) }}"
...
// Pré-carrega os destinatários já salvos (ou old() após erro de validação)
const idsIniciais = @json(
    is_array(old('clientes_ids'))
        ? array_map('intval', old('clientes_ids'))
        : array_map('intval', $campanha->destinatarios ?? [])
);
