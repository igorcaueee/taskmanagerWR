...
@section('title', 'Parcelamentos MEI — Utilitários Fiscal')
...
@section('content')
    <div class="max-w-7xl mx-auto py-6 px-4">
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold text-gray-900 dark:text-slate-100 mt-1"><i class="fa-solid fa-hand-holding-dollar"></i> Parcelamentos MEI</h1>
            <p class="text-gray-700 dark:text-gray-300">Histórico de parcelamentos do MEI, detalhamento e parcelas pendentes de emissão — PARCMEI, PARCMEI-ESP, PERTMEI e RELPMEI.</p>
        </div>

        {{-- ─── Parcelamentos MEI do cliente ──────────────────────────────────── --}}
        <div id="cardParcelamentosMei" class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 mb-6">
            <h2 class="font-semibold text-gray-800 dark:text-slate-200 text-sm uppercase tracking-wide flex items-center gap-2 mb-1">
                <i class="fa-solid fa-hand-holding-dollar text-brand"></i> Parcelamentos MEI do cliente
            </h2>
            <p class="text-xs text-gray-500 dark:text-slate-400 mb-3">
                Veja o histórico de parcelamentos do MEI no programa escolhido, o detalhamento de cada um (valor consolidado, parcelas pagas) e quais parcelas ainda estão pendentes de emissão.
            </p>

            <div class="flex flex-wrap gap-3 items-end mb-4">
                <div class="flex-1 min-w-60">
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Cliente</label>
                    <select id="selectClienteParcelamentosMei"
                ...
                <div>
                    ...
                    <select id="selectProgramaParcelamentosMei" class="rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-200 px-3 py-2 text-sm">
                        @foreach($programas as $programa)
                            <option value="{{ $programa }}">{{ $programa }}</option>
                        @endforeach
                    </select>
                </div>
                <button type="button" id="btnBuscarParcelamentosMei"
                ...
                <button type="button" id="btnBuscarParcelasMeiPendentes"
            ...
            <div id="parcelamentosMeiErro" class="hidden text-sm text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg px-3 py-2 mb-3"></div>

            <div id="parcelamentosMeiTabelaWrapper" class="hidden overflow-x-auto mb-4">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    ...
                    <tbody id="parcelamentosMeiTabelaBody" class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700 text-sm text-gray-700 dark:text-gray-300"></tbody>
                </table>
            </div>

            <div id="parcelasMeiPendentesWrapper" class="hidden overflow-x-auto">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    ...
                    <tbody id="parcelasMeiPendentesTabelaBody" class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700 text-sm text-gray-700 dark:text-gray-300"></tbody>
                </table>
            </div>
        </div>
    </div>
    ...
    <script>
    const selectClienteParcelamentosMei   = document.getElementById('selectClienteParcelamentosMei');
    const selectProgramaParcelamentosMei  = document.getElementById('selectProgramaParcelamentosMei');
    const btnBuscarParcelamentosMei       = document.getElementById('btnBuscarParcelamentosMei');
    const btnBuscarParcelasMeiPendentes   = document.getElementById('btnBuscarParcelasMeiPendentes');
    const parcelamentosMeiErro            = document.getElementById('parcelamentosMeiErro');
    const parcelamentosMeiTabelaWrapper   = document.getElementById('parcelamentosMeiTabelaWrapper');
    const parcelamentosMeiTabelaBody      = document.getElementById('parcelamentosMeiTabelaBody');
    const parcelasMeiPendentesWrapper     = document.getElementById('parcelasMeiPendentesWrapper');
    const parcelasMeiPendentesTabelaBody  = document.getElementById('parcelasMeiPendentesTabelaBody');

    function situacaoParcelamentoMeiBadge(situacao) {
    ...
    btnBuscarParcelamentosMei.addEventListener('click', async function () {
        const clienteId = selectClienteParcelamentosMei.value;
        const programa = selectProgramaParcelamentosMei.value;
        ...
        parcelamentosMeiErro.classList.add('hidden');
        parcelamentosMeiTabelaWrapper.classList.add('hidden');

        try {
            const url = new URL('{{ route('simples-nacional.parcelamentos-mei.pedidos') }}');
            ...
            if (!resp.ok || data.error) {
                parcelamentosMeiErro.textContent = data.error ?? 'Falha ao buscar parcelamentos.';
                parcelamentosMeiErro.classList.remove('hidden');
                return;
            }

            parcelamentosMeiTabelaBody.innerHTML = '';

            if (!data.pedidos || data.pedidos.length === 0) {
                parcelamentosMeiTabelaBody.innerHTML = '<tr><td colspan="4" class="px-4 py-6 text-center text-gray-500">Nenhum parcelamento encontrado para este cliente/programa.</td></tr>';
            }
            ...
            (data.pedidos ?? []).forEach(pedido => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="px-4 py-2 whitespace-nowrap">${situacaoParcelamentoMeiBadge(pedido.situacao)}</td>
                ...
                btnDetalhe.addEventListener('click', async () => {
                    ...
                    try {
                        const respDetalhe = await fetch('{{ route('simples-nacional.parcelamentos-mei.detalhe') }}', {
                            method: 'POST',
                ...
                parcelamentosMeiTabelaBody.appendChild(tr);
            });

            parcelamentosMeiTabelaWrapper.classList.remove('hidden');
        } catch (e) {
            parcelamentosMeiErro.textContent = 'Erro de comunicação com o servidor.';
            parcelamentosMeiErro.classList.remove('hidden');
        } finally {
            this.disabled = false;
    ...
    btnBuscarParcelasMeiPendentes.addEventListener('click', async function () {
        const clienteId = selectClienteParcelamentosMei.value;
        const programa = selectProgramaParcelamentosMei.value;
        ...
        this.textContent = 'Buscando...';
        parcelamentosMeiErro.classList.add('hidden');
        parcelasMeiPendentesWrapper.classList.add('hidden');

        try {
            const url = new URL('{{ route('simples-nacional.parcelamentos-mei.pendentes') }}');
            url.searchParams.set('cliente_id', clienteId);
            url.searchParams.set('programa', programa);
            ...
            if (!resp.ok || data.error) {
                parcelamentosMeiErro.textContent = data.error ?? 'Falha ao buscar parcelas pendentes.';
                parcelamentosMeiErro.classList.remove('hidden');
                return;
            }

            parcelasMeiPendentesTabelaBody.innerHTML = '';

            if (!data.parcelas || data.parcelas.length === 0) {
                parcelasMeiPendentesTabelaBody.innerHTML = '<tr><td colspan="3" class="px-4 py-6 text-center text-gray-500">Nenhuma parcela pendente de emissão.</td></tr>';
            }
            ...
            (data.parcelas ?? []).forEach(item => {
                ...
                btnEmitir.addEventListener('click', async () => {
                    ...
                    try {
                        const respEmitir = await fetch('{{ route('simples-nacional.parcelamentos-mei.emitir-das') }}', {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json', 'Content-Type': 'application/json' },
                ...
                parcelasMeiPendentesTabelaBody.appendChild(tr);
            });

            parcelasMeiPendentesWrapper.classList.remove('hidden');
        } catch (e) {
            parcelamentosMeiErro.textContent = 'Erro de comunicação com o servidor.';
            parcelamentosMeiErro.classList.remove('hidden');
        } finally {
    ...
    protegerComConfigSerpro('cardParcelamentosMei');
    </script>
    @endpush
@endsection
