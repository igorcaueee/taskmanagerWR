@php
    $isEditing = !is_null($possibilidade);
    $action = $isEditing ? route('possibilidades.update', $possibilidade->id) : route('possibilidades.save');
    $title = $isEditing ? 'Editar Possibilidade' : 'Nova Possibilidade';
@endphp
    ...
    <div class="space-y-4">
        <div>
            ...
            <input name="nome" type="text"
                   ...
                   value="{{ old('nome', $isEditing ? $possibilidade->nome : '') }}"
                   required>
        </div>
        ...
        <div>
            ...
            <input name="descricao" type="text"
                   class="mt-1 block w-full border rounded px-3 py-2"
                   value="{{ old('descricao', $isEditing ? $possibilidade->descricao : '') }}"
                   placeholder="Breve descrição da possibilidade">
        </div>

        <div>
            <label class="inline-flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                <input name="ativo" type="checkbox" class="rounded border-gray-300"
                       {{ old('ativo', $isEditing ? $possibilidade->ativo : true) ? 'checked' : '' }}>
                Ativo
            </label>
        </div>
    </div>
