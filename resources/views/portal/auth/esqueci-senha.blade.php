...
<head>
    ...
    <title>Esqueci minha senha — Portal WR Assessoria</title>
...
<div class="min-h-screen flex items-center justify-center py-16">
    <div class="w-full max-w-md">
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-xl shadow-sm border border-gray-200 dark:border-[#334155] p-8">
            <h2 class="text-lg font-semibold text-gray-700 dark:text-slate-200 mb-2">Esqueci minha senha</h2>
            <p class="text-sm text-gray-500 dark:text-slate-400 mb-6">
                Informe o e-mail cadastrado no seu acesso. Se houver um acesso vinculado a ele, enviaremos um link para redefinir a senha.
            </p>
            ...
            <form method="POST" action="{{ route('portal.password.email') }}" class="space-y-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">E-mail cadastrado</label>
                    <input
                        type="email"
                        name="email"
                        value="{{ old('email') }}"
        ...
        <p class="text-center mt-4">
            <a href="{{ route('portal.login') }}" class="text-xs text-[#0084AA] hover:text-[#006e8e] transition">
                ← Voltar para o login
            </a>
        </p>
    </div>
