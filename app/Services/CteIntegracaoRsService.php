    ...
    public function sincronizarChunk(CertificadoContabilidade $certificado, Cliente $cliente, ?int $nsuInicio = null, bool $modoBackfill = false): array
    {
        ...
        try {
            ...
            while ($lotes < self::MAX_LOTES_POR_CHUNK) {
                ...
                // parava no primeiro buraco e nunca alcançava os documentos seguintes.
                if (empty($resp['docs'])) {
...
XML;
    ...
    private function normalizarDocumento(string $nsu, string $chave, string $schema, string $xml, string $cnpjCliente): array
    {
        ...
        // são os do emitente. Já o CT-e completo tem vários grupos com CNPJ/xNome
        // (emit, rem, dest, exped, receb, toma4), então "emitente" precisa vir
        // escopado a <emit> — senão pega o primeiro nó do documento (ex.: toma4).
