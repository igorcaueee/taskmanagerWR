...
class NfseService
{
    ...
    public function buscarPorPeriodoChunk(ClienteCertificadoNfse $certificado, string $dataInicio, string $dataFim, int $nsuInicio = 0): array
    {
        ...
        try {
            ...
            // A varredura acima já seleciona os documentos pela data de PROCESSAMENTO
            // no ADN (único filtro que a API permite via NSU) — é o mesmo critério que
            // o Portal Nacional usa na tela "Notas emitidas" (coluna "Geração"). Não
            // refiltramos pela data de emissão real do XML aqui: ela só serve pra
            // exibição na tabela — refiltrar por ela descartava notas cuja Geração
            // estava dentro do período mas a Emissão real caía em outro mês (ex.:
            // nota emitida em julho e só processada/disponibilizada em agosto),
            // fazendo o total divergir do Portal.
        } finally {
